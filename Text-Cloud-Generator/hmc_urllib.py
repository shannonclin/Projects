...
its components."""
    ...
    def __repr__(self):
        ...
        return urlparse.urlunparse([self.__scheme, self.__site, self.__path, \
...
robots.txt file."""
        if not self.__robotDictionary.has_key(self.__site):
            robotRules = robotparser.RobotFileParser()
...
class WebPage(htmllib.HTMLParser):
...
server, parses it, and makes its elements available to users."""
    ...
    def __init__(self, url):
        ...
        if not self.__url.isCrawlable() or  self.__url.isForbidden():
            raise StandardError, "Attempt to fetch non-crawlable page"
        ...
        try:
            ...
            self.__rawContents = urlFile.read()
...
the fragment."""
        ...
        if location != ''  or  path == '':
            ...
            #
            return urlparse.urlunparse((scheme, location, path, parameters,
              query, fragment))
        else:
            #
            ...
            if path[0] == '/':
                ...
                if baseDir == '':
                    ...
                    if lastSlash > 0:
                        ...
                        #
                        baseDir = '/'
                return urlparse.urlunparse((scheme, location,
